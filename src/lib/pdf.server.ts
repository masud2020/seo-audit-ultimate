...
export async function buildAuditPdf(report: Report, recs: AiRec[], brand?: Brand | null): Promise<Uint8Array> {
  ...
  page.drawRectangle({ x: 0, y: H - headerBandH, width: W, height: headerBandH, color: rgb(bp[0], bp[1], bp[2]) });
  page.drawRectangle({ x: 0, y: H - headerBandH - 3, width: W, height: 3, color: rgb(ba[0], ba[1], ba[2]) });
...
export async function buildSiteAuditPdf(
  ...
  mark("Cover & Summary");
  {
    ...
    page.drawRectangle({ x: 0, y: H - bandH - 3, width: W, height: 3, color: rgb(ba[0], ba[1], ba[2]) });
    page.drawText(brandName(brand).toUpperCase(), { x: M, y: H - 34, size: 10, font: bold, color: rgb(0.85, 0.90, 1) });
    page.drawText("Whole-Site SEO Audit Report", { x: M, y: H - 66, size: 22, font: bold, color: rgb(1, 1, 1) });
    const genTag = new Date(s.finished_at ?? Date.now()).toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" });
    const gW = font.widthOfTextAtSize(genTag, 10);
    page.drawText(genTag, { x: W - M - gW, y: H - 34, size: 10, font, color: rgb(0.75, 0.82, 0.95) });
    y = H - bandH - 28;
  }
  text(sanitize(data.start_url), { size: 14, bold: true });
  ...
  text(`Generated by ${brandName(brand)} for ${data.start_url}${brand?.footer_text ? " · " + brand.footer_text : ""}`, { size: 9, color: [0.5, 0.5, 0.5] });
