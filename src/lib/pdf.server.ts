...
export async function buildAuditPdf(report: Report, recs: AiRec[]): Promise<Uint8Array> {
  ...
  text(`Domain: ${hostname}`, { size: 9, color: [0.5, 0.5, 0.5] });
  text(`Audited: ${new Date(report.fetched_at).toLocaleString()}`, { size: 9, color: [0.5, 0.5, 0.5] });
  text(`Report generated: ${generatedAt.toLocaleString()}`, { size: 9, color: [0.5, 0.5, 0.5] });
  text(`Response: HTTP ${report.meta.status_code}  ·  ${report.meta.duration_ms}ms  ·  ${(report.meta.bytes/1024).toFixed(1)} KB`, { size: 9, color: [0.5, 0.5, 0.5] });
  text("Generated by SEO Audit Tool", { size: 9, color: [0.5, 0.5, 0.5] });
  spacer(14);
  rule();
  spacer(6);
